...
/** @typedef {{ id: number, student_assesment_id: number, status: 'submitted' | 'completed', started_at: string, completed_at: string, sub_score: number, answers: any[] }} Attempt */
...
const Separator = ({ className = 'my-4' }) => <div className={`h-px bg-gray-200 ${className}`} />;
        ...
        <nav className="text-sm text-gray-500 mb-6">
            {breadcrumbs.map((item, index) => (
                <React.Fragment key={index}>
                    <a href={item.href} className="text-blue-600 hover:text-blue-800">{item.title}</a>
...
const QuestionRenderer = ({ question, answers }) => {
    ...
    switch (question.type) {
        ...
        case 'short_answer':
            ...
            const pointsEarned = answer.points_earned !== null ? parseFloat(answer.points_earned).toFixed(2) : 'N/A';
            ...
            const correctAnswerOption = question.options[0]?.option_text || 'N/A';
            ...
            if (isScored) {
                scoreLabel = isCorrect ? 'Scored (Correct)' : 'Scored (Incorrect)';
                scoreClass = isCorrect ? highlightCorrect : highlightIncorrect;
            } else if (question.type === 'fill_blank' && correctAnswerOption !== 'N/A') {
            ...
            return (
                <div className="space-y-4">
                    <p className={`${textBase} mb-2`} dangerouslySetInnerHTML={questionTextHtml}></p>

                    <Card className="shadow-none border border-gray-200">
                        <CardContent className="p-4 space-y-3">
                            <div className="flex justify-between items-start">
                                <span className="text-sm font-medium text-gray-500">Points: {isScored ? `${pointsEarned} / ${maxPoint.toFixed(2)}` : 'N/A'}</span>
                            ...
                            <div className="space-y-1">
                                ...
                                <div className={`p-3 rounded-lg border ${highlightNeutral} whitespace-pre-wrap`}>
                                    {studentText}
                                </div>
                            </div>

                            {/* Correct Answer (Only show for fill_blank/auto-scored types) */}
                            {question.type === 'fill_blank' && correctAnswerOption !== 'N/A' && (
                                <div className="space-y-1">
                                    <p className="text-sm font-semibold text-green-700">Expected Answer:</p>
                                    <div className={`p-3 rounded-lg border ${highlightCorrect} whitespace-pre-wrap`}>
                                        {correctAnswerOption}
                                    </div>
                                </div>
                            )}
        ...
        case 'matching':
            ...
            const studentPairings = answers.map(answer => {
                const leftOption = question.options.find(opt => opt.id === answer.option_id);
                if (leftOption) {
            ...
            const matchesCorrect = studentPairings.filter(p => p.isMatchCorrect).length;
            const matchesTotal = studentPairings.length;
            const pointsPerMatch = matchesTotal > 0 ? totalMatchingPoints / matchesTotal : 0;
            const matchingPointsEarned = (matchesCorrect * pointsPerMatch); // Calculate earned points

            // Determine scoring status badge
            const isMatchingScored = answers.some(a => a.points_earned !== null); // Check if any part was scored
            let matchingScoreLabel = 'Awaiting Auto-Score';
            let matchingScoreClass = highlightNeutral;
            if (isMatchingScored || matchesTotal > 0) { // Assume scored if we have answers and can calculate matches
                if (matchesCorrect === matchesTotal) {
                    ...
                    matchingScoreClass = "bg-yellow-100 text-yellow-800 border-yellow-300";
                } else {
                ...
                <div className="space-y-4">
                    ...
                    <Card className="shadow-none border border-gray-200">
                        <CardContent className="p-4 space-y-4">
                            <div className="flex justify-between items-start">
                                <span className="text-sm font-medium text-gray-500">Points: {matchingPointsEarned.toFixed(2)} / {totalMatchingPoints.toFixed(2)}</span>
                                <span className={`px-3 py-1 text-xs font-semibold rounded-full ${matchingScoreClass}`}>
                                    {matchingScoreLabel}
                                </span>
                            </div>
                            ...
                            <div className="text-sm font-semibold text-gray-700 mb-2">Submitted Matches ({matchesCorrect}/{matchesTotal} correct):</div>
                            <div className="grid gap-3">
                                {studentPairings.map((pairing, index) => (
                                    <div
                                        key={index}
                                        // Highlight the container based on correctness
                                        className={`flex flex-col sm:flex-row justify-between p-4 rounded-lg border shadow-sm transition duration-150 ${pairing.isMatchCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50'}`}
                                    >
                                        {/* Left Side (Question Part) */}
                                        <div className="flex-1 min-w-0 pr-4 mb-2 sm:mb-0">
                                            <p className="font-medium text-gray-800 truncate">{pairing.leftText}</p>
                                        </div>

                                        {/* Right Side (Submitted/Correct Answer) */}
                                        <div className="flex-shrink-0 text-right space-y-1">
                                            <div className={`text-sm font-mono p-1 rounded-md border text-left ${pairing.isMatchCorrect ? highlightCorrect : highlightIncorrect}`}>
                                                <span className="font-bold mr-2">Submitted:</span> {pairing.submitted}
                                            </div>
                                            {!pairing.isMatchCorrect && (
                                                <div className={`text-xs p-1 rounded-md border text-left ${highlightCorrect} opacity-80 mt-1`}>
                                                    <span className="font-bold mr-2 text-green-700">Correct:</span> {pairing.correct}
                                                </div>
                                            )}
                                        </div>
                                    </div>
            ...
            <div className="max-w-4xl mx-auto space-y-8 p-4">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex justify-between items-center">
                            {assessment.title}
                            <span className="text-lg font-bold px-3 py-1 rounded-full bg-blue-100 text-blue-600 shadow-md">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-4">
                            <DetailItem label="Assessment Type" value={assessment.type.charAt(0).toUpperCase() + assessment.type.slice(1)} />
                            <DetailItem label="Total Questions" value={assessment.questions.length} />
                            <DetailItem label="Total Max Points" value={totalMaxScore.toFixed(2)} />
                            <DetailItem label="Attempt Score" value={`${attemptScore} / ${totalMaxScore.toFixed(2)}`} />
                            <DetailItem label="Start Time" value={formatDate(attempt.started_at)} />
                            <DetailItem label="Completion Time" value={formatDate(attempt.completed_at)} />
                        </div>
                    </CardContent>
                </Card>

                <h3 className="text-2xl font-extrabold text-gray-900 pt-6">Question Breakdown</h3>
                ...
                <div className="space-y-10">
                    {assessment.questions.map((question, index) => {
                        ...
                        return (
                            <Card key={question.id} className="border-l-4 border-blue-500 shadow-xl">
                                <CardHeader className="flex flex-row items-center justify-between bg-gray-50 rounded-t-xl">
                                    <h4 className="text-lg font-bold text-gray-800">
                                        Question {displayIndex}:
                                    </h4>
                                    <div className="text-sm font-medium text-gray-600">
                                        Max Points: <span className="font-semibold text-blue-700">{parseFloat(question.point).toFixed(2)}</span>
                                    </div>
                                </CardHeader>
                                <CardContent>
                                    <QuestionRenderer
