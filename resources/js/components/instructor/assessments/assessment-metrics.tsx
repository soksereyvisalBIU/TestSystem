...
export default function AssessmentMetrics({
    ...
    const stats = useMemo((): MetricStats => {
        ...
        attempts.forEach((attempt) => {
            ...
            // 1. Calculate submission & review counts
            // Using a Set-like check for speed
            if (status === 'submitted' || status === 'scored' || status === 'checked') {
            ...
            // 2. Accumulate total score safely
            if (!isNaN(score)) {
                result.totalScore += score;
            }
        });

        // Final Calculations (Pre-calculated to avoid logic in JSX)
        result.average = result.totalScore / total;
        result.rate = (result.submittedCount / total) * 100;

        return result;
    }, [attempts]);

    // Pre-calculate visual states for performance
    const isFullySubmitted = stats.rate === 100;
    const isHighPerforming = stats.average > (totalMarks * 0.7);
    const hasPendingAction = stats.pendingReview > 0;
    ...
    return (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 transition-all duration-300">
            <MetricCard
                label="Submission Rate"
                value={`${Math.round(stats.rate)}%`}
                subtext={`${stats.submittedCount}/${attempts.length} students`}
                icon={CheckCircle2}
                variant={isFullySubmitted ? "success" : "default"}
            />
            ...
            <MetricCard
                ...
                subtext={`Out of ${totalMarks} pts`}
                icon={BarChart3}
                trend={isHighPerforming ? 'up' : 'down'}
            />
            ...
            <MetricCard
                ...
                subtext={hasPendingAction ? "Requires action" : "All caught up"}
                icon={AlertCircle}
                color={hasPendingAction ? "text-orange-500" : "text-slate-400"}
                className={hasPendingAction 
                    ? "ring-2 ring-orange-500/20 bg-orange-50/50 animate-in fade-in duration-500" 
